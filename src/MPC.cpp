...
class FG_eval {
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 0; i < N; i ++) {
      fg[0] += vars[cte_start+i] * vars[cte_start+i];  // cte error cost
      fg[0] += vars[epsi_start+i] * vars[epsi_start+i]; // orientation error cost
      fg[0] += (vars[v_start+i] - ref_v) * (vars[v_start+i] - ref_v); // going at reference speed

      if (i < N-1) {
        if (i < N-2) {
          fg[0] += (vars[a_start+i+1] - vars[a_start+i]) *
                    (vars[a_start+i+1] - vars[a_start+i]); // reduce bang bang
          fg[0] += 10 * (vars[delta_start+i+1] - vars[delta_start+i]) *
                    (vars[delta_start+i+1] - vars[delta_start+i]); // reduce bang bang
        }
        fg[0] += vars[delta_start+i] * vars[delta_start+i];
        fg[0] += vars[a_start+i] * vars[a_start+i];
      }
    }
    //
    // Setup Constraints
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  vector<double> result(n_vars);
  for (int i = 0; i < n_vars; i++) result[i] = solution.x[i];
  return result;
}
